...
!!! question "我应该如何配置使用VS Code的图形化调试功能? launch.json又是什么东西? " 

    出于思维的惯性和一些文档的引导, 很多同学在初次使用VS Code的时候，会尝试配置图形化的编译、调试等界面。但遇到一些复杂的需求(例如如何从文件输入), 同学们就会遇到障碍。
    
    "图形化界面易用" 是大量精心设计的、现成的图形化界面给同学们形成的错觉。在VS Code配置图形化调试界面, 其实并不是简单地“使用图形化界面”，而是更接近于通过launch.json等配置文件, 自己定制/开发一个图形化界面。

    实际上, 如果不熟悉从命令行使用g++编译, gdb调试的一系列操作, 我们很难理解VS Code的图形化调试界面如何运作。

    因此，在初学阶段，建议同学们优先掌握：直接从终端命令行敲命令，编译运行、调试的操作。在熟悉命令行的操作之后，再来尝试配置VS Code的图形化调试界面。
